...
const startBot = async () => {
  try {
    ...
    conn.ev.on('messages.upsert', async ({ messages }) => {
      ...
      //   textMessages = message.message.conversation;
      // } else if (message.message.extendedTextMessage?.text) {
      //   textMessages = message.message.extendedTextMessage.text;
      // } else if (message.message.ephemeralMessage?.message?.conversation) {
      //   textMessages = message.message.ephemeralMessage.message.conversation;
      // } else if (message.message.ephemeralMessage?.message?.extendedTextMessage?.text) {
      //   textMessages = message.message.ephemeralMessage.message.extendedTextMessage.text;
      // }
      // const fromJid = message.key.remoteJid;
      // if (textMessages?.toLowerCase() === 'getjid' && fromJid?.endsWith('@newsletter')) {
      //   await conn.sendMessage(fromJid, {
      //     text: `✅ Ini JID channel tempat kamu posting: *${fromJid}*`
      //   }, { quoted: message });
      //   return;
      // }
